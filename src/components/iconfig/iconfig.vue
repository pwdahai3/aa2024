<template>
  <div class="quickconfig-wrapper">
    ...
    <div>
      <div v-if="dynamiclevelData.length==0 && urlsuffixData.length==0" style="margin-top:20px;text-align:center;">
        暂无数据
      </div>
      <div v-else>
         <table  class="table-container">
          <tr v-for="(item,index) in dynamiclevelData" :key="'acc-'+index">
            <td>{{item.key}}</td>
            <td>
              <ul>
                <li class="tagli" v-for="(tag, tagIndex) in item.tag_name_aggregation.buckets" :key="tagIndex">
                   {{ tag.key | customFilter}}
                </li>
              </ul>
            </td>
            <!-- <td width="40" align="center">
              <a href="javascript:;" class="editdel" @click="detailDialogShow">详情</a>
            </td> -->
            <td>
              <a href="javascript:;" class="editdel" @click="editShow(item.key,'accurtate','acc-'+index)">编辑</a><a href="javascript:;" class="editdel" @click="deleteItem(item,'accurtate',index)">删除</a>
            </td>
          </tr>
          <tr v-for="(item,index1) in urlsuffixData" :key="'pat-'+index1">
            <td colspan="2">{{item.key}}</td>
            <!-- <td width="40" align="center">
              <a href="javascript:;" class="editdel" @click="detailDialogShow">详情</a>
            </td> -->
            <td width="80">
              <a href="javascript:;" class="editdel" @click="editShow(item.key,'path','pat-'+index1)">编辑</a><a href="javascript:;" class="editdel" @click="deleteItem(item,'path',index1)">删除</a>
            </td>
          </tr>
    ...
    <el-dialog
      ...
      <iframe 
      ref="child"
      src="https://192.168.2.52/grafana/d/fe779232-2d64-4537-9e4c-daa258e8104e/shadow?orgId=1&refresh=30s" 
      width="500" 
      height="500" 
      scrolling="no"
      frameborder='0'>
        ...
        if (valid) {
          ...
          client.search({
            ...
            body:{
                ...
                "aggs": {
                  "dynamic_level_path_aggregation": {
                    "terms": {
                      "script": {
                        "source": "String path = doc['shadow.access.org_url'].value;int level = params.level;int start = 0;for (int i = 0; i < level; i++) {if (start != -1) {start = path.indexOf('/', start + 1);} else {break;}}return start != -1 ? path.substring(0, start) : path;",
                        "lang": "painless",
                        "params": {
                          "level": 2
                        }
                ...
                "url_suffix_aggregation": {
                  "terms": {
                    "script": {
                      "source": "String url = doc['shadow.access.org_url'].value;int lastIndex = url.lastIndexOf('.');return lastIndex != -1 ? url.substring(lastIndex) : 'No suffix';",
                      "lang": "painless"
                    },
                    "size": 10
                  }
            ...
            if(resAgg.dynamic_level_path_aggregation){
              this.dynamiclevelData=resAgg.dynamic_level_path_aggregation.buckets;
            }
            if(resAgg.url_suffix_aggregation){
              this.urlsuffixData=resAgg.url_suffix_aggregation.buckets;
            }
