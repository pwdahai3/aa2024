    ...
    <el-table  
      ...
      <el-table-column v-for="(items, index) in tabheadDatas"
        ...
        <template slot-scope="scope, $index" class="menuTd">
          <span v-show='items.propName!="shadowUrl"'> {{scope.row[`${items.propName}`]}}</span>
          <div v-show='items.propName=="shadowUrl"'>
      ...
      <template slot-scope="scope">
        <el-button type="text" size="small" @click="getDetail(scope.row)">{{$t('lg.detail')}}</el-button>
      </template>
    </el-table-column>
    </el-table>
    <!-- </div> -->
...
export default {
  props:{
    isShowPage:{//判断当前组件是被谁引入，主要用于当前组件改变样式 为true表示被自定义图表customChartGridLayout、customChartPanel这两个组件引入
      type:Boolean
    },
    ischart:{//判断当前组件是被谁引入，主要用于当前组件改变样式 为true表示被自定义图表customChartGridLayout、customChartPanel这两个组件引入
      type:Boolean
    },
    aggsField:{//传入聚合字段，如果有聚合字段，则表格不需要加入时间列。 被自定义图表customChartGridLayout、customChartPanel这两个组件引入 时才传入aggsField
      type:String,
      default:null
    },
    tableTitle:{
    ...
    // 父组件传入的一些特有功能  access日志=>用户请求的Url  允许该明文URL被访问功能
    showSDURLMenu:{
      type:Function
    },
...
logpop
  ...
  data() {
    return {
      ...
      detailStr:{},
      tabCurPage:1,
      maxNum:10000,//es查询最多展示10000条数据
      tabheadDatas:[],
      anewFlag:true,//自定义图表引用组件时、切换表头字段时element table表体不能及时渲染，设置v-if解决
    }
  },
  computed:{
    totalNums(){
  ...
  methods:{
    ...
    getPaginationlogs(){
      this.$emit("getChangePageLogData",{curPage:this.tabCurPage,pageNum:this.tabPageNum})
    },
    showSdurlMenu(index){//显示 允许该明文url被访问 菜单
      this.showSDURLMenu(index)
    },
    addSubFilterEx(item){//允许该明文url被访问:（先调业务节点数量接口，拿到接口：‘Shadow sub filter/ex指令增加-a指令’的参数snodeId，再调用该接口）
      // console.log(item)
      var _this=this;
    ...
    downloadAutiLog(){
     ...
     let base64querystr=Base64.encode(data);
      ...
      var filename="审计日志"+"_"+year1+"-"+month1+"-"+day1+".csv";
      axios({
					  url:url,
            method: 'get',
            responseType: 'blob'
