<template>
  <div class="verticalBarChart-wrapper">
    <el-row :gutter="10">
      <el-col :span="8">
        <el-form :model="verticalBarChartForm" :rules="verticalBarChartFormRules" ref="verticalBarChartForm" label-width="120px">
          <div class="form-bg">
            <p class="chartformTitle">X-Axis</p>
            <el-form-item label="Aggregation" prop="aggregationX">
              <el-select v-model="verticalBarChartForm.aggregationX" style="width:60%">
                <el-option v-for="item in aggregationXOptions" :label="item.label" :value="item.value" :key="item.value"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="Field" prop="fieldX" v-if="verticalBarChartForm.aggregationX">
              <el-select v-model="verticalBarChartForm.fieldX" style="width:70%">
                <el-option v-for="item in fieldXOptions" :label="item.value" :value="item.value" :key="item.value" :disabled="item.disabled"></el-option>
              </el-select>
            </el-form-item>
            <div v-if="verticalBarChartForm.aggregationX=='date_histogram'">
              <el-form-item label="Interval" prop="intervalNumX">
                <el-input v-model.trim="verticalBarChartForm.intervalNumX" :placeholder="$t('lg.interval')" style="width:45%" :disabled="verticalBarChartForm.intervalLimitAble"></el-input>
                <el-select v-model="verticalBarChartForm.intervalUnitX" :placeholder="$t('lg.select')" style="width:35%" @change="intervalLimit(verticalBarChartForm.intervalUnitX)">
                <el-option
            ...
            <!-- <div v-if="verticalBarChartForm.aggregationX=='terms'">  
              <el-form-item label="Order By" prop="orderByX">
                <el-select v-model="verticalBarChartForm.orderByX" placeholder="请选择" style="width:60%">
                  <el-option
              ...
              <el-form-item label="Size" prop="sizeX">
                <el-input v-model.trim="verticalBarChartForm.sizeX" placeholder="size" style="width:45%"></el-input>
              </el-form-item>
              <el-form-item label="Order" prop="orderX">
                <el-select v-model="verticalBarChartForm.orderX" placeholder="请选择" style="width:60%">
                  <el-option
          ...
          <div class="form-bg">
            ...
            <div v-for="(items,index) in verticalBarChartForm.verticalBarChartYAxis" class="addItemBox" :key="index">
              <span class="delItemStyle" @click="deleteItem(index)"><i class="el-icon-close"></i></span>
              <el-form-item label="Aggregation" :prop="'verticalBarChartYAxis.' + index + '.aggregationY'" :rules="[{required: true, message: '不能为空', trigger: 'blur'}]">
              ...
              <el-form-item label="Field" :prop="'verticalBarChartYAxis.' + index + '.fieldY'" :rules="[{required: true, message: '不能为空', trigger: 'blur'}]" v-if="items.aggregationY && items.aggregationY!='count'">
          ...
          <div class="form-bg">
            <el-button size="small" type="primary" icon="fa fa-play" @click="getVerticalBarChartData"> {{$t('lg.generateview')}}</el-button>
      ...
      <el-col :span="16">
        <div class="chartBox">
          <vertical-bar-pic :isShow="myChartFlag" :itemParams="itemParams" :isVerticalBarChart="true" v-on:changeFlag='fatherChange'></vertical-bar-pic>
        </div>
    ...
    <el-dialog :title="$t('lg.saveviewrecord')" :visible.sync="isSetChartShow" width="45%" :before-close="setChartClose"><!--保存记录-->
      <el-form :model="setChartForm" :rules="setChartFormRules" ref="setChartForm" label-width="80px">
        ...
        <el-form-item>
          <el-button size="small" type="primary" @click="setVerticalBarChartData"> {{$t('lg.yes')}}</el-button>
        </el-form-item>
      </el-form>
    ...
    return{
     ...
     intervalOptions:[{
      ...
      verticalBarChartForm:{
        ...
        verticalBarChartYAxis:[{
          aggregationY:'',
          fieldY:''
        }]
      },
      verticalBarChartFormRules:{
        aggregationX:[
  ...
  methods:{
    intervalLimit(val){
      if(val=='w' || val=="M" || val=="y"){//周，年，月，数值只能为1
        this.verticalBarChartForm.intervalLimitAble=true;
        this.verticalBarChartForm.intervalNumX=1;
      }else{
        this.verticalBarChartForm.intervalLimitAble=false;
      }
    },
    addItem(){
      this.verticalBarChartForm.verticalBarChartYAxis.push({
        aggregationY:'',
        fieldY:''
      })
    },
    deleteItem(index){
      if(this.verticalBarChartForm.verticalBarChartYAxis.length==1){
        return;
      }
      this.verticalBarChartForm.verticalBarChartYAxis.splice(index,1);
    },
    aggregationYchange(index){
      this.verticalBarChartForm.verticalBarChartYAxis[index].fieldY="";
    },
    ...
    getVerticalBarChartData(){
      this.$refs['verticalBarChartForm'].validate((valid) => {
        if (valid) {
          const _this=this;
          ...
          const yAxixArray = _this.verticalBarChartForm.verticalBarChartYAxis;
          let paramsYArry = [];
          ...
          const params = {
            ...
            x:{
              agg:_this.verticalBarChartForm.aggregationX,
              field:_this.verticalBarChartForm.fieldX,
              interval:_this.verticalBarChartForm.intervalNumX+_this.verticalBarChartForm.intervalUnitX,
              min_doc_count:1
            },
            y:paramsYArry
          };
    ...
    setVerticalBarChartData(){//保存记录确认
