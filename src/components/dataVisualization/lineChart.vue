<template>
  <div class="lineChart-wrapper">
    <el-row :gutter="10">
      <el-col :span="8">
        <el-form :model="lineChartForm" :rules="lineChartFormRules" ref="lineChartForm" label-width="120px">
          <div class="form-bg">
            <p class="chartformTitle">X-Axis</p>
            <el-form-item label="Aggregation" prop="aggregationX">
              <el-select v-model="lineChartForm.aggregationX" style="width:60%">
                <el-option v-for="item in aggregationXOptions" :label="item.label" :value="item.value" :key="item.value"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="Field" prop="fieldX" v-if="lineChartForm.aggregationX">
              <el-select v-model="lineChartForm.fieldX" style="width:70%">
                <el-option v-for="item in fieldXOptions" :label="item.value" :value="item.value" :key="item.value" :disabled="item.disabled"></el-option>
              </el-select>
            </el-form-item>
            <div v-if="lineChartForm.aggregationX=='date_histogram'">
              <el-form-item label="Interval" prop="intervalNumX">
                <el-input v-model.trim="lineChartForm.intervalNumX" :placeholder="$t('lg.interval')" style="width:45%" :disabled="lineChartForm.intervalLimitAble"></el-input>
                <el-select v-model="lineChartForm.intervalUnitX" :placeholder="$t('lg.select')" style="width:35%" @change="intervalLimit(lineChartForm.intervalUnitX)">
                <el-option
            ...
            <!-- <div v-if="lineChartForm.aggregationX=='terms'">  
              <el-form-item label="Order By" prop="orderByX">
                <el-select v-model="lineChartForm.orderByX" placeholder="请选择" style="width:60%">
                  <el-option
              ...
              <el-form-item label="Size" prop="sizeX">
                <el-input v-model.trim="lineChartForm.sizeX" placeholder="size" style="width:45%"></el-input>
              </el-form-item>
              <el-form-item label="Order" prop="orderX">
                <el-select v-model="lineChartForm.orderX" placeholder="请选择" style="width:60%">
                  <el-option
          ...
          <div class="form-bg">
            ...
            <div v-for="(items,index) in lineChartForm.lineChartYAxis" class="addItemBox" :key="index">
              <span class="delItemStyle" @click="deleteItem(index)"><i class="el-icon-close"></i></span>
              <el-form-item label="Aggregation" :prop="'lineChartYAxis.' + index + '.aggregationY'" :rules="[{required: true, message: $t('lg.required'), trigger: 'blur'}]">
              ...
              <el-form-item label="Field" :prop="'lineChartYAxis.' + index + '.fieldY'" :rules="[{required: true, message: $t('lg.required'), trigger: 'blur'}]" v-if="items.aggregationY && items.aggregationY!='count'">
          ...
          <div class="form-bg">
            <el-button size="small" type="primary" icon="fa fa-play" @click="getLineChartData"> {{$t('lg.generateview')}}</el-button>
      ...
      <el-col :span="16">
        <div class="chartBox">
          <line-pic :isShow="myChartFlag" :itemParams="itemParams" :isLineChart="true" v-on:changeFlag='fatherChange'></line-pic>
        </div>
    ...
    <el-dialog :title="$t('lg.saveviewrecord')" :visible.sync="isSetChartShow" width="45%" :before-close="setChartClose"><!--保存记录-->
      <el-form :model="setChartForm" :rules="setChartFormRules" ref="setChartForm" label-width="80px">
        ...
        <el-form-item>
          <el-button size="small" type="primary" @click="setLineChartData"> {{$t('lg.yes')}}</el-button>
        </el-form-item>
      </el-form>
...
export default {
  data(){
    return{
      ...
      itemParams:{},//存储视图参数传给linePic子组件
    }
  },
  methods:{
    intervalLimit(val){
      if(val=='w' || val=="M" || val=="y"){//周，年，月，数值只能为1
        this.lineChartForm.intervalLimitAble=true;
        this.lineChartForm.intervalNumX=1;
      }else{
        this.lineChartForm.intervalLimitAble=false;
      }
    },
    addItem(){
      this.lineChartForm.lineChartYAxis.push({
        aggregationY:'',
        fieldY:''
      })
    },
    deleteItem(index){
      if(this.lineChartForm.lineChartYAxis.length==1){
        return;
      }
      this.lineChartForm.lineChartYAxis.splice(index,1);
    },
    aggregationYchange(index){
      this.lineChartForm.lineChartYAxis[index].fieldY="";
    },
    ...
    getLineChartData(){
      this.$refs['lineChartForm'].validate((valid) => {
        if (valid) {
          const _this=this;
          ...
          const yAxixArray = _this.lineChartForm.lineChartYAxis;
          let paramsYArry = [];
          ...
          const params = {
            ...
            x:{
              agg:_this.lineChartForm.aggregationX,
              field:_this.lineChartForm.fieldX,
              interval:_this.lineChartForm.intervalNumX+_this.lineChartForm.intervalUnitX,
              min_doc_count:1
            },
            y:paramsYArry
          };
