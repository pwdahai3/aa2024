      ...
      function getRegionSunburstData(type){
        ...
        client.search({
          ...
          buckets.forEach(element=>{
            var countNum=element.doc_count || 0;
            var cityName=element.total_count.value.loc.city_name||`[${element.total_count.value.loc.location.lon},${element.total_count.value.loc.location.lat}]`
            seriesData.push({name:cityName,value:countNum})
          })
      ...
      function getDomainPieData(){
        client.search({
          ...
          _this.domainPieChart.on("click",(params)=>{
            ...
            _this.popQueryObj.bool.must.push({
                  "query_string": {
                      "query":"shadow.access.domain:"+_this.logDialogTitle.replace(/:/g,'\\:')
                    }
            });
            _this.getOnepieceFromPie();
          })
