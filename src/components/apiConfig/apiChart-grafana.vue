...
<template>
  <div class="apiChart-wrapper">
    ...
    <iframe src="https://192.168.2.52/grafana/d/fe779232-2d64-4537-9e4c-daa258e8104e/shadow?orgId=1" frameborder="0" style="width:100%; height:100%"></iframe>
    ...
    <div style="margin-top:30px">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <el-button type="primary" @click="generateConf">{{$t('lg.search')}}</el-button>
        </el-col>
      </el-row>
      </el-form>
    </div>
    <div>
      <h2 class="h2title">{{$t('lg.jinrionlineapi')}}</h2>
      <div v-if="todayData.length==0" style="margin-top:20px;text-align:center;">
        {{$t('lg.nodata')}}
      </div>
      <div v-else>
         <table  class="table-container">
          <tr v-for="(item,index) in todayData" :key="'acc-'+index">
            <td>{{item.key}}</td>
          </tr>
         </table>
      </div>
    </div>
    <div style="margin-top:20px; border-top:1px dashed #ccc;padding-top:28px">
      <h2 class="h2title">{{$t('lg.jinriofflineapi')}}</h2>
      <div v-if="yesterdayData.length==0" style="margin-top:20px;text-align:center;">
        {{$t('lg.nodata')}}
      </div>
      <div v-else>
         <table  class="table-container">
          <tr v-for="(item,index1) in yesterdayData" :key="'pat-'+index1">
