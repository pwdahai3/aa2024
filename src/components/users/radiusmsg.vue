  ...
  <div class="users-wrapper">
    ...
    <div class="usersContent">
      <table>
        <thead>
          <tr>
            <th>{{$t('lg.radiusserverip')}}</th>
            <th>{{$t('lg.port')}}</th>
            <th>{{$t('lg.shouquanma')}}</th>
            <th>{{$t('lg.validdate')}}（{{$t('lg.second')}}）</th>
            <th width="280"></th>
          </tr>
        </thead>
        <tbody>
          <tr>
            ...
            <td> {{users && users.port}}</td>
            <td> {{users && users.authCode}}</td>
            <td> {{users && users.timeout}}</td>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td> {{shadownodeData && shadownodeData.port}}</td>
            <td> {{shadownodeData && shadownodeData.authCode}}</td>
            <td> {{shadownodeData && shadownodeData.timeout}}</td>
            <td class="text-center">
    ...
    <radiusedit :editShow="editShow" @eidtclose="editShow=false" :ntpData="users" @hasadduser="getCmRadius"></radiusedit>
    <shadowradiusedit :shadownode="shadownodeVal"  :editShow="shadoweditShow" @eidtclose="shadoweditShow=false" :ntpData="shadowntpData"  @haseditshadow="getShadownodeData"></shadowradiusedit>
...
import radiusedit from './radiusEdit.vue'
import shadowradiusedit from './shadowradiusEdit.vue';
export default {
  components:{
    radiusedit,
    loading,
  ...
  methods:{
    ...
    getShadownodeData(item){
      ...
      var url=SERVER_API_URL+"pe/shadow/radiusConf/get/"+sdid;
    ...
    getCmRadius(){
      var url=SERVER_API_URL+"pe/shadow/radiusConf/get";
      axios.get(url).then((res)=>{
        if(res.status==200){
          var resArray=res.data.data;
          this.users=resArray;
