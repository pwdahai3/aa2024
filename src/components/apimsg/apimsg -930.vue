<template>
  ...
  <div style="position:relative; width:100%">
    <div style="position:absolute;right:0; top:4px; z-index:100 ; display:flex" class="urlsearchbox">
      <el-input v-model.trim="urlsearch" placeholder="api查询"></el-input> &nbsp;&nbsp; <el-button type="primary" size="mini">{{$t('lg.search')}}</el-button>
    </div>
        ...
        <div v-else>
          <table class="table-container">
            ...
            <tbody>
              ...
              <td style="position:relative; padding-right:90px">
                {{item._source && item._source.shadow && item._source.shadow.access && item._source.shadow.access.shadow_url}}
                  <a href="javascript:;" class="configa" @click="toggleA(item,index)">{{$t('lg.peizhi')}}</a>
                  <div class="adivul" :style="{width:(isencn=='en'?'640px':'320px')}" v-if="aMenu===index">
                    <ul class="aul">
                      <li class="configli"><a href="javascript:;" class="configlia" @click="oneLinkShow(item)">{{$t('lg.onelink')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="shadow_request_time_shownew(item)">{{$t('lg.shoudongdlsz')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="shadow_verify_browser_shownew(item)">{{$t('lg.browserParams')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="shadow_verify_agent_shownew(item)">{{$t('lg.userBhIndct')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="userbehaviorintercept_shownew(item)">{{$t('lg.yonghuxwlj')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="jsinsert_shownew(item)">{{$t('lg.jstrong')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="verifyTokenVal_shownew(item)">{{$t('lg.dongtaizwgn')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="encryptKeySwitch_shownew(item)">{{$t('lg.bodyenfun')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="verifyCookieMd5Val_shownew(item)">{{$t('lg.cookiefcfhgn')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="setRejectHeaderVal_shownew(item)">{{$t('lg.jujuetouyugn')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="accessdenyVal_shownew(item)">{{$t('lg.xiaxiang')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="botconfVal_shownew(item)">{{$t('lg.saomiaofygn')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="paramverify_shownew(item)">{{$t('lg.cansjmqzyz')}}</a></li>
                      <li class="configli"><a href="javascript:;" class="configlia" @click="defenseCookieVal_shownew(item)">{{$t('lg.shadowcookieyz')}}</a></li>
                    </ul>
...
<div style="margin-top:30px" v-if="isBeginSearch">
  ...
  <div style="margin-top:16px">
    <el-row>
      ...
      <el-col :span="12">
        <div class="chart-content" style="margin-left:8px">
    ...
    <el-row>
      <el-col :span="24">
        <div class="chart-content" style="margin-right:8px;">
          ...
          <div class="centercon2" style="height:auto;">
...
export default {
  ...
  methods:{
      ...
      client.search({
           ...
           // console.log(res);
            if(res.aggregations && res.aggregations.response_code_buckets && res.aggregations.response_code_buckets.buckets){
              ...
              this.cuowu2xx=resaggbuck["2xx"].doc_count || "--";
              this.cuowu4xx=resaggbuck["4xx"].doc_count || "--";
              this.cuowu5xx=resaggbuck["5xx"].doc_count || "--";
            }
          });
    },
    shibailv(url,dm,nd){
    ...
    defenseCookieVal_shownew(item){
const _this=this;
      this.onShow(item,_this.defenseCookieVal_show=true)
    },
    paramverify_shownew(item){
const _this=this;
      this.onShow(item,_this.botconfVal_show=true)
    },
    botconfVal_shownew(item){
const _this=this;
      this.onShow(item,_this.botconfVal_show=true)
    },
    accessdenyVal_shownew(item){
      const _this=this;
      this.onShow(item,_this.accessdenyVal_show=true)
    },
    setRejectHeaderVal_shownew(item){
      const _this=this;
      this.onShow(item,_this.setRejectHeaderVal_show=true)
    },
    verifyCookieMd5Val_shownew(item){
      const _this=this;
      this.onShow(item,_this.verifyCookieMd5Val_show=true)
    },
    encryptKeySwitch_shownew(item){
      const _this=this;
      this.onShow(item,_this.encryptKeySwitch_show=true)
    },
    verifyTokenVal_shownew(item){
      const _this=this;
      this.onShow(item,_this.verifyTokenVal_show=true)
    },
    jsinsert_shownew(item){
      const _this=this;
      this.onShow(item,_this.jsinsert_show=true)
    },
    userbehaviorintercept_shownew(item){
      const _this=this;
      this.onShow(item,_this.userbehaviorintercept_show=true)
    },
    shadow_verify_agent_shownew(item){
      const _this=this;
      this.onShow(item,_this.shadow_verify_agent_show=true)
    },
    shadow_verify_browser_shownew(item){
      const _this=this;
      this.onShow(item,_this.shadow_verify_browser_show=true)
    },
...
if(!resovledata){//该location对应的shadow节点上的Shadow Cookie验证功能开启才能设置location的Shadow Cookie验证功能
    ...
    shadow_request_time_shownew(item){
      const _this=this;
      this.onShow(item,_this.shadow_request_time_show=true)
    },
    ...
    onShow(item,visible=false){
      this.getShadowNodes2().then((resolvedata,reject)=>{
        ...
        this.curOneLinkItem.locationName=item._source.shadow.access.shadow_url;
        visible=true;
      }) 
    },
    oneLinkShow(item){
      const _this=this;
      this.onShow(item,_this.oneLinkVisible=true)
    },
...
.configli{ height: 26px; line-height: 26px; flex: 50%; box-sizing: border-box; border-bottom: 1px solid #777;}

.configli:nth-child(odd){ border-right: 1px solid #777;}
.configli:last-child{
  border-right: none;
}
.configli:nth-last-child(-n+2){
  border-bottom: none;
}
a.configlia{ color: #fff; display: block; padding: 0 6px;}
a.configlia:hover{ background-color: #888;}
...
#apiMngChartid1,#eCodeTypeContainer,#orgIpLineContainer,#domainLineContainer,#orgIpPieContainer,#domainPieContainer,#fingerPieContainer,#autoToolPieContainer,#regionBarContainer{
  width: 100%;
  height: 310px;
  padding: 10px;
  box-sizing: border-box;
}
.h4title{ font-size: 18px; text-align: center; padding: 12px 0;}
