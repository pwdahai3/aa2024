...
<div>
  <div class="quickconfig-wrapper">
    ...
    <div style="margin:12px 0">
      <el-button type="primary" @click="apiexport">导出</el-button>
      <el-button type="primary" @click="apiimportPop">导入</el-button>
    </div>
  ...
  <div style="position:relative; width:100%">
    ...
    <el-tabs v-model="apitabactiveName" @tab-click="handleClick">
      <el-tab-pane :label="$t('lg.apilist')" name="first">
        ...
        <p class="nodatapop">{{$t('lg.nodata')}}</p>
        </div>
        <div v-else>
...
<div style="margin-top:30px" v-if="isBeginSearch">
  ...
  <div style="margin-top:16px">
    <el-row>
      <el-col :span="12">
        <div class="chart-content" style=" margin-right:8px; position:relative;">
          <h4 class="h4title">{{switchIpTit}}</h4> 
          <div class="switchpos">
            <img src="/static/icon/switch.png" width="18px" alt="" @click="apimngChart5('click')"> 
          </div>
          <div class="centercon2">
      ...
      <el-col :span="12">
        <div class="chart-content" style="margin-left:8px; position:relative">
          <h4 class="h4title">{{switchIpTit2}}</h4>
          <div class="switchpos">
            <img src="/static/icon/switch.png" width="18px" alt="" @click="apimngChart4('click')"> 
          </div>
          <div class="centercon2">
            <div v-if="normalIPRank.length==0" class="nodata4">
              暂无数据
            </div> 
            <ul>
          ...
          <div class="centercon2">
            ...
            <ul>
              <li v-for="item in shibaidiaoyonglvarr" class="flexli">
                ...
                <span class="flexlispan1">{{item.key}}</span>
                <span class="flexlispan2">{{item.value | percentFormat}}</span>
              </li>
            </ul>
          </div>
        </div>
...
<el-dialog width="900px" :title="curAttrpopItemTit" :visible.sync="attrpop_show" :close-on-click-modal="false" :before-close="attrpop_showClose">
  ...
  <el-dialog  :title="$t('lg.daochu')"  :visible.sync="exportVisible"  width="40%"  :before-close="exportpopClose">
      <el-form ref="queryPanelform2" class="queryPanelBox" style="padding-bottom:16px" :model="queryPanelform2" :rules="queryPanelformRules2"  label-width="100px">
      <el-row :gutter="10">
        <el-col :span="12">
          <el-select v-model="shadownodesform22.shadownodes" :placeholder="$t('lg.select')" @visible-change="getShadowNodes22()" style="width:100%">
            <template v-for="n in apiAllnodes">
              <el-option :label="n" :value="n"></el-option>
            </template>
          </el-select>
        </el-col>
        <el-col :span="12">
            <el-select  v-model="queryPanelform2.checkDomain" :placeholder="$t('lg.selectdomain')" @visible-change="getDomains22()" style="width:100%">
                <template v-for="item in apiAlldms">
                  <el-option :key="item.domainName+':'+item.domainPort" :label="item.domainName+':'+item.domainPort" :value="item.domainName+':'+item.domainPort"></el-option>
                </template>
              </el-select>
        </el-col>
      </el-row>
      <div style="margin-top:14px">
        <el-button type="primary" @click="apiexport">{{$t('lg.daochu')}}</el-button>
      </div>
    </el-form>
  </el-dialog>
  <el-dialog  :title="$t('lg.daoru')"  :visible.sync="importVisible"  width="40%"  :before-close="exportpopClose">
      <el-form ref="queryPanelform2" class="queryPanelBox" style="padding-bottom:16px" :model="queryPanelform2" :rules="queryPanelformRules2"  label-width="100px">
      <el-row :gutter="10">
        <el-col :span="12">
          <el-select v-model="shadownodesform22.shadownodes" :placeholder="$t('lg.select')" @visible-change="getShadowNodes22()" style="width:100%">
...
export default {
  ...
  data(){
    return{
      ...
      exportVisible:false,
      importVisible:false,
      switchIpTit:'恶意源IP访问排行',
      curSwtichIp:'shadow.access.remote_ip',
      switchIpTit2:'正常源IP访问排行',
      curSwtichIp2:'shadow.access.remote_ip',
      curAttrpopItemTit:'',
      curAttrpopItem:'',
      rspage:true,
    ...
    shadownodesform2:{ 
      ...
      queryPanelformRules2:{
        // checkDomain:[
        //   {required:true, message:this.$t('lg.required'), trigger:'blur'},
        // ]
      },
      targetLcFlage:true,////map begin
      targetValue:[
        {
  ...
  methods:{
    ...
    onchange(file){
      let dm=this.queryPanelform2.checkDomain.split(":")[0];
      let por=this.queryPanelform2.checkDomain.split(":")[1];
      this.uploadActionUri=SERVER_API_URL+"pe/shadow/api/es/import?snodeId="+this.shadownodesform22.shadownodes+"&domainName="+dm+"&domainPort="+por;
    },
    submitUpload() {
      const _this=this;
      if(!this.shadownodesform22.shadownodes){
        _this.$message.error('请选择节点');
        return;
      }
      if(!this.queryPanelform2.checkDomain){
        _this.$message.error('请选择域名');
        return;
      }
    ...
    exportpopClose(){
      this.apiAllnodes=[];
      this.apiAlldms=[];
      this.shadownodesform22.shadownodes="";
      this.queryPanelform2.checkDomain="";
      this.exportVisible=false;
      this.importVisible=false;
    },
    getShadowNodes22(){//获取节点
      var _this=this;
      var nodesurl=SERVER_API_URL+'pe/shadow/wireshark/snodelist';
     this.$refs.loading.showLoading();
     axios.get(nodesurl).then((res)=>{
        if(res.data.responseCode == 0){
          const resData = res.data.data;
          this.apiAllnodes=resData;
    ...
    getDomains22(){
      ...
      var nodesurl=SERVER_API_URL+'pe/shadow/domain/all?snodeId='+_this.shadownodesform22.shadownodes;
     this.$refs.loading.showLoading();
     axios.get(nodesurl).then((res)=>{
        if(res.data.responseCode == 0){
          const resData = res.data.data;
          this.apiAlldms=resData;
          this.$refs.loading.hideLoading();
        }else{
          _this.$message.error(_this.$t('lg.failed')+': '+ res.data.data.message);
        }
    ...
    apiallnodes(){
      return new Promise((resolve,reject)=>{
        const url=SERVER_API_URL+'pe/shadow/wireshark/snodelist';
    ...
    apialldomains(){
      return new Promise((resolve,reject)=>{
        const url=SERVER_API_URL+'pe/shadow/domain/all';
        axios.get(url).then((res)=>{
          if(res.data.responseCode==0){
            const resData = res.data.data;
            this.apiAlldms=resData;
    ...
    apiexport(){
      const _this=this;
          ...
          var url=SERVER_API_URL+"pe/shadow/api/es/export?fromTimestamp="+fromTime+"&toTimestamp="+endTime;
      if(this.shadownodesform.shadownodes){
        let hoststr="&hostNameList="+encodeURI([_this.shadownodesform.shadownodes]);
        url+=hoststr;
      }
      if(this.queryPanelform.checkDomain){   
        let dmnoport=this.queryPanelform.checkDomain;
        let dmstr="&domainNameList="+encodeURI([dmnoport]);
        url+=dmstr;
      }
      if(this.urlsearch){   
        let lstr="&locationName="+this.urlsearch;
        url+=lstr;
      }
      ...
      axios({
        ...
        const fileName = "API数据"+"_"+year+""+month+""+day+".xlsx";
  ...
  function getOrgIpPieData(type){// 绘制图表
      ...
      client.search({
           ...
           // console.log(res)
             if(res.aggregations.org_url_buckets && res.aggregations.org_url_buckets.buckets){
              let tmpArr=res.aggregations.org_url_buckets.buckets || [];
              let tmpArr2=tmpArr.map((item)=>{
                return{
                  key:item.key,
                  value:item.non_200_count.doc_count/item.total_count.value
                } 
              });
              tmpArr2.sort((a,b)=>{
            ...
            //  console.log(tmpArr2)
              this.shibaidiaoyonglvarr=tmpArr2;
    ...
    shibaidiaoyonglv2(item){
      ...
      client.search({
           ...
           // console.log(res)
             ...
             // console.log(tmpArr2)
              this.shibaidiaoyonglvarr2=tmpArr2;
    ...
    apimngChart5(isclick){
      ...
      if(isclick=="click"){
        if(_this.curSwtichIp=="shadow.access.remote_ip"){
          _this.curSwtichIp="shadow.access.org_remote_ip";
          _this.switchIpTit='恶意代理IP访问排行';
        }else{
          _this.curSwtichIp="shadow.access.remote_ip";
          _this.switchIpTit='恶意源IP访问排行';
        }        
      }
      client.search({
            index: "shadow-filebeat-warn*",
    ...
    apimngChart4(isclick){
      ...
      let esDm=this.queryPanelform.checkDomain || "*";
      if(isclick=="click"){
        if(_this.curSwtichIp2=="shadow.access.remote_ip"){
          _this.curSwtichIp2="shadow.access.org_remote_ip";
          _this.switchIpTit2='正常代理IP访问排行';
        }else{
          _this.curSwtichIp2="shadow.access.remote_ip";
          _this.switchIpTit2='正常源IP访问排行';
        }        
      }
      client.search({
            index: "shadow-filebeat-*",
