<template>
  <div>
    <el-dialog width="45%" :title="domainObj.domainName+':'+domainObj.port" :visible.sync="wafmodalShow" :close-on-click-modal="false" :before-close="wafstatusmodalClose">
      ...
      <el-form ref="wafmodalform" label-width="90px">
        ...
        <el-form-item>
          <el-button type="primary" @click="saveWaf">{{$t('lg.save')}}</el-button>
          <el-button @click="wafstatusmodalClose">{{$t('lg.cancel')}}</el-button>
        </el-form-item>
      </el-form>
      <div v-show="wafmodalLoading">
...
export default{
  ...
  data(){
    return{
      ...
      wafmodalLoading:false
    }
  },
  watch:{
    'wafmodalShow':function(){
      if(this.wafmodalShow){
        if(this.data.waf=="on"){
  ...
  methods:{
    wafstatusmodalClose(){
      this.$emit("setbysavewafmodal",false);
    },
    showCheckRuleset(){
