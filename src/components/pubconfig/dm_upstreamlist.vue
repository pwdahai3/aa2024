...
<template>
  <div>
    <el-dialog width="55%" :title="domainObj.domainName+':'+domainObj.port" :visible.sync="isShow" :close-on-click-modal="false" :before-close="upstreamlistmodalClose">
      ...
      <div>
        ...
        <!-- 服务器健康管理 -->
        <p class="myServerCheck"><span>{{$t('lg.serverhealth')}}：<i :class="enableScCk?'el-icon-success font-green':'el-icon-warning'"></i> {{enableScCk?$t('lg.opened'):$t('lg.unopened')}}</span><span class="changeBalance font-blue" @click="changeSverCheck()"><i :class="isShowServerCheckBox?'el-icon-caret-top':'el-icon-caret-bottom'"></i>{{$t('lg.healthExam')}}</span></p>
        <div v-show="isShowServerCheckBox" class="serverCheckBox">
          <el-form :model="editSvCkfrom" ref="editSvCkfrom" :label-width="$store.state.isencn=='en'?'230px':'175px'" :rules="editSvCkfromRules">
            <el-form-item :label="$t('lg.checkinterval')" prop="scInterval">
              <el-row>
                <el-col :span="16">
                  <el-input v-model.trim="editSvCkfrom.scInterval" auto-complete="off" :placeholder="$t('lg.plzenter')" style="width:98%"></el-input>
                </el-col>  
                <el-col :span="8">
                  <el-select v-model="scIntervalUnitVal" :placeholder="$t('lg.select')" style="width:100%">
            ...
            <el-form-item :label="$t('lg.serverstopcounts')" prop="scFall">
              <el-input v-model.trim="editSvCkfrom.scFall" auto-complete="off" :placeholder="$t('lg.plzenter')"></el-input>
            </el-form-item>
            <el-form-item :label="$t('lg.serverstartcounts')" prop="scRise">
              <el-input v-model.trim="editSvCkfrom.scRise" auto-complete="off" :placeholder="$t('lg.plzenter')"></el-input>
            </el-form-item>

            <el-form-item :label="$t('lg.timeoutset')" prop="scTimeout">
              <el-row>
                <el-col :span="16">
                  <el-input v-model.trim="editSvCkfrom.scTimeout" :placeholder="$t('lg.plzenter')" style="width:98%"></el-input>
                </el-col>
                <el-col :span="8">
                  <el-select v-model="scTimeoutUnitVal" :placeholder="$t('lg.select')" style="width:100%">
                    <template v-for="item in scTimeIntervalUnit">
                      <el-option :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </template>
                  </el-select>
            ...
            <el-form-item :label="$t('lg.protocolused')" prop="scTypeUnit">
              <el-select v-model="editSvCkfrom.scType" style="width:50%">
            ...
            <el-form-item :label="$t('lg.httpcheck')" v-if="editSvCkfrom.scType=='http'" prop="checkHttpSendTxt">
              <el-input :placeholder="$t('lg.plzenter')" v-model="editSvCkfrom.checkHttpSendTxt">
                <el-select v-model="editSvCkfrom.checkHttpSendType" slot="prepend" :placeholder="$t('lg.select')" style="width:90px">
            ...
            <el-form-item :label="$t('lg.httpcheckexptected')" label-width="170px" v-if="editSvCkfrom.scType=='http'" prop="checkHttpExpectAlive">
              <el-select v-model="editSvCkfrom.checkHttpExpectAlive" :placeholder="$t('lg.select')" style="width:50%" multiple>
            ...
            <el-form-item>
              <el-button type="success" @click="editSvCkEnsure(true)" v-if="!enableScCk">{{$t('lg.on')}}</el-button>
              <el-button type="primary" @click="editSvCkEnsure(true)" v-if="enableScCk">{{$t('lg.edit')}}</el-button>
              <el-button type="info" @click="editSvCkEnsure(false)" v-if="enableScCk">{{$t('lg.off')}}</el-button>
            </el-form-item>
          </el-form>
        </div>
        ...
        <div class="gradeSettingBox">
          ...
          <div v-show="isShowHgSettingBox" class="gradeSettinginner">
            <el-form :model="editGradeSetform" ref="editGradeSetform" :label-width="$store.state.isencn=='en'?'180px':'100px'" :rules="editGradeSetformRules">
              <el-form-item :label="$t('lg.ntlmlong')" prop="ntlmValule">
                <el-input v-model.trim="editGradeSetform.ntlmValule" auto-complete="off" :placeholder="upstreamObj.ntlmVal?$t('lg.inputntlmlong'):$t('lg.nontlmlong')" clearable></el-input>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" @click="editGSEnsure('editGradeSetform')">{{$t('lg.yes')}}</el-button>
              </el-form-item>
            </el-form>
          </div>
        </div>
    ...
    <!-- 编辑弹框 -->
    <el-dialog width="40%" :title="domainObj.domainName+':'+domainObj.port" :visible.sync="isShowEditCmd" :close-on-click-modal="false" :before-close="isEditCmdClose">
    ...
    <!-- 权重弹框 -->
    <el-dialog width="40%" :title="domainObj.domainName+':'+domainObj.port" :visible.sync="isIpWeight" :close-on-click-modal="false" :before-close="isweightCmdformClose">
      <el-row>
...
export default{
  ...
  data(){
    return{
      ...
      editSvCkfromRules:{
        scInterval:[
          {required:true, message:this.$t('lg.required'), trigger:'blur'},
          {pattern:/^[1-9]\d*$/,message: this.$t('lg.fsdayu0'), trigger: 'change'}
        ],
        scFall:[
          {required:true, message:this.$t('lg.required'), trigger:'blur'},
          {pattern:/^[1-9]\d*$/,message: this.$t('lg.fsdayu0'), trigger: 'change'}
  ...
  methods:{
    ...
    editCmdEnsure(formname){//确认编辑
      this.$refs[formname].validate((valid) => {
        if(valid){
          ...
          var domainPort=this.domainObj.port;
          // console.log(this.editCmdOldform);
          // console.log(this.editCmdform);
    ...
    changeBalanceEnsure(){//确认更改
      ...
      if(balanceType=="xff_hash"){
        balanceType="xxf_hash"
      }
