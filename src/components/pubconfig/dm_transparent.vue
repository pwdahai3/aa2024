  ...
  <div>
    <el-dialog width="40%" :title="domainObj.domainName+':'+domainObj.port" :visible.sync="isShow" :close-on-click-modal="false" :before-close="limReqmodalClose">
      <el-form ref="dmTransparentform" :model="dmTransparentform"  label-width="100px">
        ...
        <el-form-item>
          <el-button type="primary" @click="submitTransparent">{{$t('lg.save')}}</el-button>
  ...
  methods:{
    getDmtransparentData(){
      return new Promise((resolve,reject)=>{
        ...
        if(_this.isAuto){
          _this.url=SERVER_API_URL+"pe/shadow/domain/iptransparent?snodeId="+snodeId+"&domainName="+domainName+"&domainPort="+domainPort+"&autoconfig=true";
        }else if(_this.isEditConfig){
          _this.url=SERVER_API_URL+"pe/shadow/domain/iptransparent?snodeId="+snodeId+"&domainName="+domainName+"&domainPort="+domainPort+"&savedConfig=true";
        }else{
          _this.url=SERVER_API_URL+"pe/shadow/domain/iptransparent?snodeId="+snodeId+"&domainName="+domainName+"&domainPort="+domainPort;
        }
        axios.get(_this.url).then((res)=>{
          if(res.data.responseCode==0){
            var resData=res.data.data;
            _this.dmTransparentform.ipTransparent=resData.ipTransparent=="on"?true:false;
            resolve(resData);
          }else{
    ...
    limReqmodalClose(){
      this.$emit("setbydmtransparentmodal",false);
    ...
    submitTransparent(){
      var _this=this;
        ...
        axios.patch(_this.url,{
          ipTransparent:_this.dmTransparentform.ipTransparent==true?"on":"off"
        }).then((res)=>{
          if(res.data.responseCode==0){
              this.$message({
                type: "success",
