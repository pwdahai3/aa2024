...
<template>
  <div>
    <el-dialog width="55%" :title="shadowObj.name+' ['+shadowObj.snodeId+']'" :visible.sync="isShow" :close-on-click-modal="false" :before-close="sdGlbInstructClose">
      <div class="httpDctBox">
       ...
       <div class="httpDctBox" style="margin-top:26px">
        <el-button class="addGIBtn" type="primary" size="mini" @click="stream_addGIDialogShow()">{{$t('lg.xinzstreampz')}}</el-button>
      ...
      <div class="globalDctBox">
        <el-button class="addGIBtn" type="primary" size="mini" @click="addGIDialogShow(false)">{{$t('lg.xinzglobal')}}</el-button>
        <h1 class="directiveTitle">{{$t('lg.globalzdypz')}}</h1>
        <p class="nodata-box" v-show="directiveData.globalDctList.length==0">{{$t('lg.nodata')}}</p>
        <table class="table-bg" v-show="directiveData.globalDctList.length>0">
          <thead>
            <tr>
              <th>{{$t('lg.peizhimc')}}</th>
              <th>{{$t('lg.peizhics')}}</th>
          ...
          <tbody>
            <tr v-for="item in directiveData.globalDctList">
              <td>{{item.dctName}}</td>
              <td>{{item.dctParam}}</td>
              <td class="text-center">
    ...
    <!-- 增加自定义指令弹框 -->
    <el-dialog width="65%" :title="directiveHttp?shadowObj.name+' ['+shadowObj.snodeId+']--'+$t('lg.xinzhttpzdypz'):shadowObj.name+' ['+shadowObj.snodeId+']--'+$t('lg.xinzglobalzdypz')" :visible.sync="addGIDialogVisible" :close-on-click-modal="false" :before-close="addGIDialogClose">
          <el-form ref="glbInstructform" :model="glbInstructform" :rules="glbInstructformRules" :label-width="$store.state.isencn=='en'?'180px':'80px'" style="margin-right:30px">
    ...
    <!-- 编辑单条自定义指令弹框 -->
    <el-dialog width="40%" :title="directiveHttp?shadowObj.name+' ['+shadowObj.snodeId+']--'+$t('lg.xiugaihttpzdyp'):shadowObj.name+' ['+shadowObj.snodeId+']--'+$t('lg.xiugailglobalzdypz')" :visible.sync="editGIDialogVisible" :close-on-click-modal="false" :before-close="editGIDialogClose">
      <el-form :model="editInstructform" ref="editInstructform" :rules="editInstructformRules" :label-width="$store.state.isencn=='en'?'180px':'80px'">
        ...
        <el-form-item :label="$t('lg.peizhics')" prop="dctParam">
          <el-input v-model.trim="editInstructform.dctParam" :placeholder="$t('lg.plzenter')"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog width="65%" :title="shadowObj.name+' ['+shadowObj.snodeId+']--'+$t('lg.xinzstreamzdypz')" :visible.sync="stream_addGIDialogVisible" :close-on-click-modal="false" :before-close="stream_addGIDialogClose">
          <el-form ref="stream_glbInstructform" :model="stream_glbInstructform" :rules="glbInstructformRules" :label-width="$store.state.isencn=='en'?'180px':'80px'" style="margin-right:30px">
            <el-form-item :label="$t('lg.peizhimc')" prop="instructName">
              <el-input v-model.trim="stream_glbInstructform.instructName" :placeholder="$t('lg.plzenter')"></el-input>
            </el-form-item>
            <el-form-item :label="$t('lg.peizhics')" prop="instructParam">
              <el-input v-model.trim="stream_glbInstructform.instructParam" :placeholder="$t('lg.plzenter')"></el-input>
            </el-form-item>
            <el-form-item>
    ...
    <el-dialog width="65%" :title="shadowObj.name+' ['+shadowObj.snodeId+']--'+$t('lg.bianjstreamzdypz')" :visible.sync="stream_editGIDialogVisible" :close-on-click-modal="false" :before-close="stream_addGIDialogClose">
          <el-form ref="stream_glbInstructform" :model="stream_glbInstructform" :rules="glbInstructformRules" :label-width="$store.state.isencn=='en'?'180px':'80px'" style="margin-right:30px">
            <el-form-item :label="$t('lg.peizhimc')" prop="instructName">
            ...
            <el-form-item>
              <el-button type="primary" @click="stream_editsubmit('stream_glbInstructform')">{{$t('lg.save')}}</el-button>
              <el-button @click="stream_addGIDialogClose">{{$t('lg.cancel')}}</el-button>
            </el-form-item>
          </el-form>
      <loading v-show="stream_GlbInstructLoading"></loading>
      ...
      _this.streamlist_new=[];
        ...
        axios.get(url).then((res)=>{
          if(res.data.responseCode==0){
            ...
            if(resData.globalStreamDirectives){
              resData.globalStreamDirectives.forEach(item=>{
      ...
      return new Promise((resolve,reject)=>{
        ...
        axios.get(url).then((res)=>{
          ...
          if(res.data.responseCode==0){
            ...
            _this.directiveData.globalDctList=[];
            ...
            if(resData.globalDirectives){
              resData.globalDirectives.forEach(item=>{
                _this.directiveData.globalDctList.push({
                  dctName:Object.keys(item)[0],
                  dctParam:Base64.decode(item[Object.keys(item)[0]])
                })
              })
