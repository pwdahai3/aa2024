...
<template>
  <div>
    <el-dialog width="60%" :title="$t('lg.editgongjileix')" :visible.sync="isShow" :close-on-click-modal="false" :before-close="attClose">
      <el-form ref="addattfrom" :model="addattfrom" :rules="addattfromRules" label-width="100px">
        <el-form-item label="id" prop="id" style="display:none;">
          <el-input v-model.trim="addattfrom.id"></el-input>
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" @click="updateatttype">{{$t('lg.update')}}</el-button>
...
export default{
  ...
  watch:{
    "isShow":function(){
        this.addattfrom.attackType=this.data.attackType;
        this.addattfrom.remark=this.data.remark;
        this.addattfrom.score=this.data.score;
        this.addattfrom.id=this.data.id;
    }
  },
  data(){
    return{
  ...
  methods:{
    attClose(){
      ...
      this.$emit("updateattactypeclose",false);
    },
    updateatttype(){
      var _this=this;
